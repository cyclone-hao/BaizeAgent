const translation = {
  ...
  plansCommon: {
    ...
    apiRateLimitTooltip: 'API 请求频率限制涵盖所有通过 AgentFlow API 发起的调用，例如文本生成、聊天对话、工作流执行和文档处理等。',
    ...
    messageRequest: {
      ...
      tooltip: '消息额度旨在帮助您便捷地试用 AgentFlow 中的各类 OpenAI 模型。不同模型会消耗不同额度。额度用尽后，您可以切换为使用自己的 OpenAI API 密钥。',
    },
    annotatedResponse: {
      title: '{{count,number}} 个标注回复数',
      tooltip: '标注回复功能通过人工编辑标注为应用提供了可定制的高质量问答回复能力。',
    },
    ragAPIRequestTooltip: '指单独调用 AgentFlow 知识库数据处理能力的 API。',
  ...
  plans: {
    ...
    enterprise: {
      ...
      features: [
        ...
        '多个工作空间 & 企业级管理',
        'SSO',
        '由 AgentFlow 合作伙伴支持的可协商的 SLAs',
        '高级的安全 & 控制',
        '由 AgentFlow 官方提供的更新 & 维护',
        '专业技术支持',
      ],
    },
  },
