const translation = {
  ...
  plansCommon: {
    ...
    apiRateLimitTooltip: 'API レート制限は、テキスト生成、チャットボット、ワークフロー、ドキュメント処理など、白泽智能体平台 API 経由のすべてのリクエストに適用されます。',
    ...
    messageRequest: {
      ...
      tooltip: 'メッセージクレジットは、白泽智能体平台 でさまざまな OpenAI モデルを簡単にお試しいただくためのものです。モデルタイプに応じてクレジットが消費され、使い切った後はご自身の OpenAI API キーに切り替えていただけます。',
    },
    annotatedResponse: {
      title: '{{count,number}}の注釈クォータ制限',
      tooltip: '手動での回答の編集と注釈により、カスタマイズ可能な高品質の質問応答機能をアプリに提供します。（チャットアプリのみに適用）',
    },
    ragAPIRequestTooltip: '白泽智能体平台 のナレッジベース処理機能のみを呼び出す API 呼び出しの数を指します。',
