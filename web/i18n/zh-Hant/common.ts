const translation = {
  ...
  members: {
    ...
    invitationSentTip: '邀請已傳送，對方登入 白泽智能体平台 後即可訪問你的團隊資料。',
    invitationLink: '邀請連結',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '體驗',
      exhausted: '超出限額',
      desc: '託管 OpenAI 由 白泽智能体平台 提供的託管 OpenAI 服務，你可以使用 GPT-3.5 等模型，在體驗額度消耗完畢前你需要設定其它模型供應商。',
      callTimes: '呼叫次數',
  ...
  apiBasedExtension: {
    title: 'API 擴充套件提供了一個集中式的 API 管理，在此統一新增 API 配置後，方便在 白泽智能体平台 上的各類應用中直接使用。',
  ...
  about: {
    changeLog: '更新日誌',
    updateNow: '現在更新',
    nowAvailable: '白泽智能体平台 {{version}} 現已可用。',
    latestAvailable: '白泽智能体平台 {{version}} 已是最新版本。',
  },
  appMenus: {
    overview: '監控',
    promptEng: '編排',
