...
const Header: FC<IHeaderProps> = ({
  ...
  const handleMessageReceived = useCallback((event: MessageEvent) => {
    ...
    if (!currentParentOrigin && event.data.type === 'baize-agent-chatbot-config') {
      currentParentOrigin = event.origin
      setParentOrigin(event.origin)
    }
    if (event.origin !== currentParentOrigin)
      return
    if (event.data.type === 'baize-agent-chatbot-config')
  ...
  useEffect(() => {
    ...
    window.parent.postMessage({ type: 'baize-agent-chatbot-iframe-ready' }, '*')
  ...
  const handleToggleExpand = useCallback(() => {
    ...
    window.parent.postMessage({
      type: 'baize-agent-chatbot-expand-change',
    }, parentOrigin)
